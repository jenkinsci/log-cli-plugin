stage('all') {
    node('windock') {
        ...
        withEnv(["MVN=${tool('mvn')}", "WSTMP=$tmp"]) {
            bat($/
                docker run --rm -v %CD%:c:\ws -v %WSTMP%:c:\wstmp -v %MVN%:c:\mvn -v m2repo:C:\Users\ContainerAdministrator\.m2\repository openjdk:8-windowsservercore-1809 cmd /c dir /s /b C:\Users\ContainerAdministrator\.m2\repository
